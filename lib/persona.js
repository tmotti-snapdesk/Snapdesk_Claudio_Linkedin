...
// Applique les champs éditables (venant de l'admin) sur un persona existant.
export function applyPersonaEdits(base, edits) {
  const out = { ...base };
  for (const f of ['name', 'role', 'toneOfVoice', 'template', 'extraInstructions']) {
  ...
  if (Array.isArray(edits.examples)) {
    out.examples = edits.examples.map((s) => String(s).trim()).filter(Boolean);
  } else if (typeof edits.examples === 'string') {
    // Exemples séparés par une ligne vide.
    out.examples = edits.examples.split(/\n{2,}/).map((s) => s.trim()).filter(Boolean);
  }
  out.isUser = true;
  out.active = true;
  return out;
